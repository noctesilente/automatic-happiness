...
public class AuthorizationHeaderFilter extends AbstractGatewayFilterFactory<AuthorizationHeaderFilter.Config> {
    ...
    private boolean isJwtValid(String jwt) {
        ...
        try {
            subject = Jwts
                    ...
                    .getSubject();
            // 토큰에서 고유한 아이디를 꺼내려고 함
            // 하지만 모든 예외 상황들을 해결하기 위해 try catch로 감싸기
        } catch (Exception e) {
            e.printStackTrace();
            flag = false;
        }

        // subject가 없거나 비어있거나 value만 있다면?
    ...
    // 다 판별되면 그다음 단계인 user service로 보냄
}
