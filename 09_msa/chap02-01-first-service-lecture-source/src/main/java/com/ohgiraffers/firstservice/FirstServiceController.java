...
//        log.info("요청 경로 포트2: {}", env.getProperty("server.port"));
        // 화면에 바로 뜨게 간추리기

        /* 설명. first-service 어플리케이션 여러 개 실행 시(스케일 아웃) 실행되는 어플레케이션 확인(포트 번호로) */
        /* 설명. Gateway의 lb(로드 밸런싱)를 통해 RoundRobin 방식으로 스위칭 됨을 확인 */
        return "I'm ok port at " + env.getProperty("local.server.port");
    }

    /* 설명. Gateway를 거치고 넘어오는 요청 시 RequestHeader에 값이 추가됨(Gateway의 필터에 의해) */
    @GetMapping("message")
