...
public class ProblemOfUsingDirectSQLTests {
     ...
     *  4. 동일성 보장 문제 -> 똑같은 메뉴를 조회할 때마다 다루는 객체는 매번 새로워짐 -> 주소값이 달라짐
     *  -> JDBC를 가지고는 적용하기 번거롭거나 힘듦
     * */
    ...
    // 위처럼 하면 네 개 중 몇 개를 까먹거나 잊을 경우에 문제 발생 -> 케이스 바이 케이스로 매핑해야 함
    // JPA: 조회, 업데이트 -> 모든 컬럼에 대해서 일어남 -> 따로따로 관리하는 것보다 효율적임
    // -> 모든 컬럼을 다 가져와서 조회 항목 변경에 따른 의존성을 없애버리는 것

    /* 설명. 2-2. 연관된 객체 문제 */
    // MenuAndCategory에 대한 설명 -> 메뉴인데 카테고리를 가지고 있는 것을 조회할 때 -> JOIN이 얼마나 귀찮은지 알아보기
    ...
    /* 설명. 3-1. 상속 문제
     *  객체 지향 언어의 상속 개념과 유사한 것이 데이터베이스의 서브 타입 엔티티이다.(서브타입을 별도의 클래스로 나뉘었을 때)
    ...
    /* 설명. 3-2. 연관관계 문제, 객체 그래프 탐색, 방향성 문제
