...
public class ResponseEntityTestController {
    ...
    public ResponseEntity<ResponseMessage> findAllUsers() {
        ...
        // 이거 주석 처리해서 줘도 됨 -> 이 줄은 담아서 주는 거기 때문

        return new ResponseEntity<>(responseMessage, headers, HttpStatus.OK);
    }
    // 여기서는 ResponseMessage 타입으로 함
    ...
    public ResponseEntity<ResponseMessage> findUserByNo(@PathVariable int userNo) {
        ...
        // Map에 담으면 추가로 responsemessage를 안 만들어도 됨
        // Hashmap은 key와 value가 json object 형태에 들어가고 response message는 필드에 담긴 값이 json object에 들어감
        // 나머지는 똑같음!
    ...
    /* 설명.
     *  기존에 우리가 배웠던 @RequestParam과 달리 json 문자열이 핸들러 메소드로 넘어올 떄는 @RequestBody를 붙이고
    ...
    public ResponseEntity<?> registUser(@RequestBody UserDTO newUser) {
        // json 문자열로 넘어왔기 때문에 RequestParam이 아니라 RequestBody로 받아야 함
        // ? 붙인 건 바디에 실어서 넘어가는 응답값이 없을 때
        ...
        return ResponseEntity
                .created(URI.create("/entity/users/" + users.get(users.size() - 1).getNo()))        // lastUserNo + 1로 해도 됨
                .build();
    ...
    public ResponseEntity<?> modifyUser(@RequestBody UserDTO modifyInfo, @PathVariable int userNo) {
        // json 형태로 넘어왔기 때문에 requestbody로 받아주기 - http에서 받음
        // 수정할 내용 말고도 수정될 값(1번)을 보내야 돼서 그것도 받아야 함 -> @PathVariable int userNo
        // 여기서는 VO 말고 DTO로 받고 있음

        /* 설명. PathVariable로 넘어온 번호와 일치하는 회원 한 명 추출 */
        UserDTO foundUser =             // findById(userNo)orThrow~ 로 쓰는데 지금은 collection이라 이렇게 씀
