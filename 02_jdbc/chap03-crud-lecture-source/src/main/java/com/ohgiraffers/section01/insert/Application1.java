...
//        System.out.println("con = " + con);
        ...
        * */
        ...
        try {
            ...
            /* 설명.
             *  혹시라도 기존의 테이블에서 auto_increment 값이 증가되어 있다면 다시 줄이고자 할 때 잘못 들어간 뒤의 값을
             *  지우고 auto_increment를 원하는 값까지 초기화 후 insert를 해야 한다.
            ...
            * */
            ...
            // insert나 update나 delete나 다 executeUpdate 사용
            // 자동 커밋 상태에서는 executeUpdate 하면 자동 커밋된다.(feat. 수동 커밋 변환이 이루어져야 함)
            result = pstmt.executeUpdate();

            if (result > 0) {
                // 제대로 인서트가 일어나면 확정을 지어라
                System.out.println("insert 결과: " + result + " 수동 커밋하기!");
                con.commit();
            } else {
                // 즉 자동 커밋이 되었기 때문에 rollback을 써도 소용이 없음
                con.rollback();
            }
